 ...
 * @param {string} selector 可拖动元素CSS Selector
 * @param {string} fordrag 可拖动区域CSS Selector
 */
export function makeDragable(selector, fordrag) {
    ...
    forDrag.addEventListener('mousedown', (e) => {
        ...
        forDrag.classList.add('dragging')
    ...
    forDrag.addEventListener('mouseup', () => {
        ...
        forDrag.classList.remove('dragging')
    })
}
...
export function parseContent(content) {
    ...
    content = content.replace(rg, convert)

    const rgtag = /(\[.*?\])/g
    const parse = (match, tag) => {
        for (let emoji of emojis) {
            if (emoji.tag === tag) {
                return `<img class="im-bubble-emoji" src="${emoji.file}" alt="${emoji.tag}" title="${emoji.tag}" />`
            }
        }
        return tag
    }
    return content.replace(rgtag, parse)
}
